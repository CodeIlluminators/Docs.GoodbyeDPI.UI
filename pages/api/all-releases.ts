 ...
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiReleasesResponse>): Promise<void> {
  const cachedReleases = cache.get<GitHubRelease[]>(CacheSettings.ALL_RELEASES_KEY);
